...
void quick_sort_recurs(int *array, size_t size, size_t startIdx, size_t endIdx)
{
	...
	if (endIdx > startIdx)
	{
		pIdx = partition2(array, size, startIdx, endIdx);
		if (pIdx != startIdx)
...
/**
 * partition2 - function that sets a "pivot" in the array and splits it
...
int partition2(int *array, size_t size, size_t startIdx, size_t endIdx)
{
...
/**
 * pswap - function that swaps two elements in an array
